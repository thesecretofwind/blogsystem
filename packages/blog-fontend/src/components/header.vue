...
<template>
  <div>
    ...
    <div class="headImgBox" :style="{backgroundImage:globalStatus.themeObj.value.top_image?'url('+globalStatus.themeObj.value.top_image+')':'url(static/img/headbg05.jpg)'}">
      <div class="scene">
        <div><span id="luke"></span></div>
      </div>
      <div class="h-information">
  
                      <img :src="globalStatus.themeObj.value.head_portrait?globalStatus.themeObj.value.head_portrait:'static/img/tou.png'" alt="">
  
        <h2 class="h-description">
  
                          {{globalStatus.themeObj.value.autograph ? globalStatus.themeObj.value.autograph:"三更灯火五更鸡，正是男儿读书时"}}
...
import { Typeit } from '@/static/plug.js'
...
const pMenu = ref(true);
const activeIndex = ref('/');
const classListObj =  ref<Category[]>([]);
...
const globalStatus = storeToRefs(store);
const route = useRoute();

// const searchChange = () => {
// 	if(!input.value) {
// 		globalStatus.keywords = '';
// 		router.push({ path: '/'})
// 	}
// }
...
const requestCategoryList = () => {
	getCategoryList()
	.then(res => classListObj.value = res.data)
}

const logoinFun = (msg: number) => {
	localStorage.setItem('logUrl', router.currentRoute.value.fullPath);

	router.push({path: `/login?login=${msg}`});
}
...
const routeChange = () => {
	...
	if ((route.name === 'Share' || route.name === 'Home') && globalStatus.keywords) {
		input.value = globalStatus.keywords.value;
	} else {
		input.value = '';
		globalStatus.keywords.value = '';
	}
};
...
onMounted(() => {
	...
	const onVisibilitychange = () =>  {
		if (!_.get(document, hiddenProperty)) {
			if (route.path !== '/detail-share') {
				hasLogin.value = localStorage.getItem('userInfo') ? true : false;
			}
		}
	}
	...
	let timer = setTimeout(() => {
		Typeit(globalStatus.themeObj,'#luke');
		clearTimeout(timer);
	})
})
